/*
Define the behavior of all the functions of the arduinobot_interface.hpp that
we have declared.
...
CallbackReturn ArduinobotInterface::on_deactivate(const rclcpp_lifecycle::State & previous_state)
{
    ...
    if(arduino_.IsOpen())
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while closing the connection witsth the port " << port_);
            return CallbackReturn::FAILURE;
        }
